...
import {addToFavouritesAC, removeFromFavouritesAC, setFavsObj, setFavsObjAC} from '../redux/favouritesReducer'
import {splitByThree} from '../utils/splitByThree'

export const Favourites = () => {
    const dispatch = useDispatch()
    const favsId = useSelector((state) => state.favourites.favsId)
    const favouritesObj = useSelector((state) => state.favourites.favouritesObj)
    ...
    useEffect(() => {
        if (favsId.length) dispatch(setFavsObj(favsId))
        return () => {
            dispatch(setFavsObjAC([]))
        }
    }, [])
